...
public class EncounterFileRepository implements EncounterRepository {
    ...
    // Injects CSV path from src/main/resources/app.properties
    public EncounterFileRepository(@Value("${encounters.csv}") String filePath) {
    ...
    public List<Encounter> findByType(EncounterType type) throws DataAccessException {
        List<Encounter> matches = new ArrayList<>();
        if (type == null) {
            return matches;
        }
    ...
    public boolean update (Encounter encounter) throws DataAccessException {
        ...
        for (int i = 0; i < all.size(); i++) {
            if (all.get(i).getEncounterId() == encounter.getEncounterId()) {
                all.set(i, encounter);     // replace existing record
                writeAll(all);             // persist all records back to file
                return true;
            }
        }
        return false;
